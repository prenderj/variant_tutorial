...
```
Each of the -an parameters specify which features we want to use to try and separate good from bad variants. The bov1000G.vcf is a list of variants generated by the 1000 bulls sequencing consortium (http://www.1000bullgenomes.com/). We are assuming here that they are good quality variants.  We could also, for example, use the set of variants on the Illumina bovine HD array as an even better truth set. The UGBT VCF file we downloaded earlier is expected to contain many false positive variants and so is not used to train the model.
